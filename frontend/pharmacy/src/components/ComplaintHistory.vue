<template>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <tr :key="complaint.id" v-for="complaint in this.myComplaints">
                ...
                <td>
                    <button class="btn btn-primary" v-on:click="showResponse(complaint)" v-if="complaint.response">View response</button>
                </td>
            </tr>
        </tbody>
    </table>
...
export default {
    ...
    data(){
        return {
            patient: {},
            myComplaints: [],
            currentComplaint: { description: "", response: "",},
        }
    },
    methods: {
        getComplaints(){
            ComplaintDataService.getPatientsComplaints(this.patient.id).then(response => {
                this.myComplaints = response.data;

                UserDataService.getAllUsers().then(response => {
                for(let user of response.data){
                    for(let complaint of this.myComplaints){
    ...
    created(){
        this.patient = AuthService.getCurrentUser();
        this.getComplaints();
    },
}
</script>