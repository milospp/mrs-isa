<template>  
    ...
    <table>
        <tr>
            <td>Name: </td><td><input type="text" id="derName" v-model="queryParams.name" v-on:change="filterResults()"></td>
            <td>Surname: </td><td><input type="text" id="derSurname" v-model="queryParams.surname" v-on:change="filterResults()"></td>
            <td>Address: </td><td><input type="text" id="derAddress" v-model="queryParams.address" v-on:change="filterResults()"></td>
            <td>Rating: </td><td><input type ="number" id="derMinRating" v-model="queryParams.minRating" v-on:change="filterResults()" v-bind:min="0" v-bind:max="5"></td>
            <td>-</td><td><input type ="number" id="derMaxRating" v-model="queryParams.maxRating" v-on:change="filterResults()" v-bind:min="0" v-bind:max="5"></td>
        </tr>
    </table>

    <h4>Dermatologists you've visited: </h4>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <tr :key="key" v-for="(value, key) in this.results">
                ...
                <td>
                    <button class="btn btn-primary" type="submit" data-dismiss="modal" v-on:click="this.setDermatologist(value[1])" 
                    data-toggle="modal" data-target="#dermatologistComplaintModal">Complaint</button>
                </td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="modal fade" id="dermatologistComplaintModal" tabindex="-1" role="dialog" aria-labelledby="dermatologistComplaintModal" aria-hidden="true">
...
export default {
    ...
    methods: {
        setDermatologists(){
            PharmacistDataService.getVisitedPharmacists(this.author.id).then(response => {
                for(let doctor of response.data){
                    if(doctor.role == "DERMATOLOGIST"){
                        this.dermatologists.set(doctor.id.toString(), doctor);
                    }
                }

                this.results = this.dermatologists;
            });
        },
        filterResults(){
            this.results = new Map();

            for(let [key, value] of this.dermatologists){
        ...
        setDermatologist(dermatologist){
            this.complaint.doctor = dermatologist;
        },
        sendComplaint(){
