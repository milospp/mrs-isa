...
<script>
import SysAdminDataService from '../service/SysAdminDataService.js';
export default {
    name: 'AddSysAdminForm',
    data() {
        return {
            message: null,
            registerData: {
    ...
    methods: {      
        formCheck(e) {
            SysAdminDataService.SendSysAdmin(this.registerData).then(response => {
                if (response.data) {
                    this.$toast.show(
                        "System administrator successfully added!",
                        {
